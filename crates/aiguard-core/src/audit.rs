...
impl AuditLog {
        ...
        let compressed_payload: Option<Vec<u8>> = event
            ...
            .map(|p| {
                zstd::encode_all(p.as_slice(), 3)
                    .map_err(|e| AiguardError::Compression(e.to_string()))
            })
            .transpose()?;
    ...
    pub fn query_session(&self, session_id: &str) -> Result<Vec<AuditEvent>> {
        ...
        rows.map(|r| r.map_err(AiguardError::from).and_then(raw_to_event))
            .collect()
    }
    ...
    pub fn decompress_payload(compressed: &[u8]) -> Result<Vec<u8>> {
        zstd::decode_all(compressed).map_err(|e| AiguardError::Compression(e.to_string()))
    }
}
