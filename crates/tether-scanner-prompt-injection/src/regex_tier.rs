...
impl RegexTierScanner {
    pub fn new() -> aiguard_core::Result<Self> {
        let rules_file: RulesFile = toml::from_str(RULES_TOML).map_err(|e| {
            aiguard_core::TetherError::Config(format!("Failed to parse pi-rules.toml: {e}"))
        })?;
        ...
        for (rule_idx, raw) in rules_file.rule.into_iter().enumerate() {
            let regex = Regex::new(&raw.pattern).map_err(|e| {
                aiguard_core::TetherError::Config(format!("Bad regex in rule {}: {e}", raw.id))
            })?;
        ...
        let automaton = if all_keywords.is_empty() {
            AhoCorasick::builder()
                ...
                .map_err(|e| {
                    aiguard_core::TetherError::Config(format!("AhoCorasick build error: {e}"))
                })?
        } else {
            AhoCorasick::builder()
                .ascii_case_insensitive(true)
