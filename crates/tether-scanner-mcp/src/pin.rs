...
impl ToolPinner {
    ...
    pub fn approve(&self, server_id: &str, tools_json: &serde_json::Value) -> Result<()> {
        ...
        fs::create_dir_all(&self.pin_dir).map_err(|e| aiguard_core::TetherError::Io(e))?;

        let pin_path = self.pin_path(server_id);
        let json = serde_json::to_string_pretty(&record)
            .map_err(|e| aiguard_core::TetherError::Serde(e))?;

        fs::write(&pin_path, json).map_err(|e| aiguard_core::TetherError::Io(e))?;
    ...
    pub fn remove_pin(&self, server_id: &str) -> Result<()> {
        let pin_path = self.pin_path(server_id);
        if pin_path.exists() {
            fs::remove_file(&pin_path).map_err(aiguard_core::TetherError::Io)?;
        }
        Ok(())
    }
}
...
/// Default pin directory: `~/.local/share/tether/mcp-pins/`
